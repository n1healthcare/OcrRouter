"""General VLM backend implementation for document processing."""
...
from .client import GeneralVLMClient


class GeneralVLMBackend(BaseModelBackend):
    """General VLM backend for document analysis using Vision Language Models.
    ...
    Example:
        ...
        >>> backend = GeneralVLMBackend(settings)
        >>> middle_json, results = await backend.analyze(pdf_bytes)
    """

    def __init__(self, settings: Settings):
        """Initialize the General VLM backend.
    ...
    @property
    def client(self) -> GeneralVLMClient:
        """Get the General VLM client instance."""
        if self._client is None:
            self._client = GeneralVLMClient(self._settings)
        return self._client
    ...
    async def analyze(
        ...
        **kwargs: Any,
        ...
        # Validate output_mode - Gemini only supports OCR
        if output_mode == "layout_only":
            raise ValueError(
                "General VLM does not support 'layout_only' mode. "
                "Use 'ocr_only' or use CompositeBackend with a different layout_model."
            )
        if output_mode == "all":
            raise ValueError(
                "General VLM does not support 'all' mode (layout + OCR). "
                "Use 'ocr_only' or use CompositeBackend with layout_model and ocr_model='generalvlm'."
            )
        ...
        logger.debug("Running General VLM full-page OCR (ocr_only mode)")
